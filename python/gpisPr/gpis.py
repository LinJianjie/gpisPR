...
class GPISOpt:
    ...
    def step(self, target_points, T_last: Transformation = None):
        ...
        for i in tqdm.tqdm(range(self.max_iteration)):
            ...
            if (i % 1)==0:
                J=np.mean(np.abs(self.gpisModel.prediction(target_points_update)))
                ...
                if np.abs(J-J_last)<0.0000001:
                    ...
                    J_last=J
                if (np.abs(J-self.obj_opt_min)<=0.00005 ):
    ...
    def getBetaM(self,target_points):
        BetaM=self.gpisModel.kernel_(target_points,self.gpisModel.X_source)
        return BetaM
